...
def order(n, points): #turns tensor of points into integer distances along hilbert curve of itteration n
...
def hilbert_insertion(npoints = 1003, batchsize = 200):
    ...
    for i in range(batchsize):
        insertion_order[i, 3:] = order(2 ** 6, points[i]) #number of possible positions is n ** 2
...
def random_insertion(npoints = 1003, batchsize = 200):
    env.reset(npoints, batchsize, nsamples = 1)
    for i in range(npoints - 3):
